...
class Tetris(object):
	...
	def __init__(self, bx, by):
		...
		self.board_up = pygame.Rect(
			0, constants.BOARD_UP_MARGIN, self.resx, constants.BOARD_HEIGHT)
		self.board_down = pygame.Rect(
			0, self.resy-constants.BOARD_HEIGHT, self.resx, constants.BOARD_HEIGHT)
		self.board_left = pygame.Rect(
			0, constants.BOARD_UP_MARGIN, constants.BOARD_HEIGHT, self.resy)
		self.board_right = pygame.Rect(
			self.resx-constants.BOARD_HEIGHT, constants.BOARD_UP_MARGIN, constants.BOARD_HEIGHT, self.resy)
		# List of used blocks
		self.blk_list = []
		# Compute start indexes for tetris blocks
		self.start_x = math.ceil(self.resx/2.0)
		self.start_y = constants.BOARD_UP_MARGIN + \
			constants.BOARD_HEIGHT + constants.BOARD_MARGIN
		...
		self.block_data = (
			...
			([[0, 0], [0, 1], [1, 0], [1, 1]],
			 constants.ORANGE, False),  # O block
		...
		#Background Music
		winsound.PlaySound(
    		"resources/music/Tetris Theme.wav", winsound.SND_LOOP + winsound.SND_ASYNC
		)
	...
	def run(self):
		...
		self.myfont = pygame.font.SysFont(
			pygame.font.get_default_font(), constants.FONT_SIZE)
		self.screen = pygame.display.set_mode((self.resx, self.resy))
		pygame.display.set_caption("Tetris-Made By Ravanger_101")
	...
	def game_logic(self):
		...
		any_border = self.active_block.check_collision(
			[self.board_left, self.board_up, self.board_right])
	...
	def get_block(self):
		...
		if self.new_block:
			...
			self.active_block = block.Block(
				data[0], self.start_x, self.start_y, self.screen, data[1], data[2])  # This line throws an error
